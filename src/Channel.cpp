...
void Channel::removeClientFromTheChannel(int fd) {
    if (_clients.find(fd) != _clients.end()) {
        ...
        _clients.erase(fd);
        std::cout << "Client " << fd << " removed from channel " << _name << std::endl;
    } else {
        std::cerr << RED "Client " << fd << " not found in channel " << _name << RESET << std::endl;
    }
}

// Accepte un client dans le canal
void Channel::acceptClientInTheChannel(ClientRef client) {
    _clients[client.getFd()] = client;  // Assurez-vous que le client est référencé correctement
    std::cout << "Client " << client.getFd() << " added to channel " << _name << std::endl;
}
...
void Channel::receiveMessageInTheChannel(int fd) {
    if (_clients.find(fd) != _clients.end()) {
        ...
        if (!message.empty()) {
            std::cout << "Message received in channel " << _name << " from client " << fd << ": " << message << std::endl;
