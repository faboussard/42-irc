...
/*   Updated: 2024/11/30 23:18:45 by yusengok         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bot::sendResponseToServer(std::deque<BotRequest>::iterator itRequest) {
  ...
  if (itRequest->command == ADVICE) {
    ...
    if (forecast != UNKNOWN_FORECAST) {
      ...
      oss << "PRIVMSG " << itRequest->clientNickname << " :Tomorrow in " 
          << DEFAULT_CITY << "... " << "\r\n";
...
static eForecast getForecastByCode(const std::string &code);
...
eForecast Bot::parseJsonWeatherResponse(const std::string &apiResponse) {
  ...
  if (weatherCode.empty() || weatherCode.length() != 4 || weatherCode[0] != '1')
    return (UNKNOWN_FORECAST);
  return (getForecastByCode(weatherCode));
}
...
eForecast getForecastByCode(const std::string &code) {
  if (code == "1000")
    return (SUNNY);
...
bool isCodeSnowy(const std::string &code) {
  ...
  std::vector<std::string> snowyCodes(snowyCodesArray, snowyCodesArray + sizeof(snowyCodesArray) / sizeof(snowyCodesArray[0]));
  return (std::find(snowyCodes.begin(), snowyCodes.end(), code) != snowyCodes.end());
}
...
void Bot::sendAsciiCatByCommand(BotRequest *request, eBotCommand command) {
  if (command == MENU) {
    ...
    sendAsciiCat(request->clientNickname, _instructions);
...
void Bot::sendAsciiCat(const std::string &nick, const stringVector &cat) {
  ...
  for (std::vector<std::string>::const_iterator it = cat.begin();
       it != itEnd; ++it) {
    std::ostringstream oss;
    oss << "PRIVMSG " << nick << " :" << *it << "\r\n";
    sendMessageToServer(oss.str());
